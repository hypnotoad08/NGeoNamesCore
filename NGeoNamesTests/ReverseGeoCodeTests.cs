...
namespace NGeoNamesTests
{
	...
	public class ReverseGeoCodeTests
	{
		...
		public async Task ReverseGeoCode_RadialSearch_ReturnsCorrectResults()
		{
			...
			var data = (await GeoFileReader.ReadExtendedGeoNamesAsync(@"testdata\test_GB.txt").ToListAsync()).ToArray();
		...
		[TestMethod]
		public async Task ReverseGeoCode_RadialSearchAsync_ReturnsCorrectResults()
		{
			// Read a file with data with points in and around London in a 20Km radius
			var data = (await GeoFileReader.ReadExtendedGeoNamesAsync(@"testdata\test_GB.txt").ToListAsync()).ToArray();
			var rg = new ReverseGeoCode<ExtendedGeoName>(data);
		...
		[TestMethod]
		public async Task ReverseGeoCode_RadialSearchAsync_ReturnsMaxCountResults()
		{
			// Read a file with data with points in and around London in a 20Km radius
			var data = (await GeoFileReader.ReadExtendedGeoNamesAsync(@"testdata\test_GB.txt").ToListAsync()).ToArray();
			var rg = new ReverseGeoCode<ExtendedGeoName>(data);
		...
		[TestMethod]
		public async Task ReverseGeoCode_NearestNeighbourSearchAsync_ReturnsCorrectResults()
		{
			// Read a file with data with points in and around London in a 20Km radius
			var data = (await GeoFileReader.ReadExtendedGeoNamesAsync(@"testdata\test_GB.txt").ToListAsync()).ToArray();
			var rg = new ReverseGeoCode<ExtendedGeoName>(data);
